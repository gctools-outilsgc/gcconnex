...
?>
    ...
    <div class="b_extended_profile">
        ...
        // no edit buttons provided therefore no requirements to mention users the ability to edit

        if ( has_content($user, 'description') ) {
            init_ajax_block_no_edit(elgg_echo('gcconnex_profile:about_me'), 'about-me', $user);
            echo elgg_view('b_extended_profile/about-me');
            finit_ajax_block('about-me');
        }

        if ( has_content($user, 'education') ) {
            init_ajax_block_no_edit(elgg_echo('gcconnex_profile:education'), 'education', $user);
            echo elgg_view('b_extended_profile/education');
            finit_ajax_block('education');
        }

        if ( has_content($user, 'work') ) {
            init_ajax_block_no_edit(elgg_echo('gcconnex_profile:experience'), 'work-experience', $user);
        ...
        if (elgg_is_active_plugin('missions') && $user->opt_in_missions == 'gcconnex_profile:opt:yes' 
            && (strstr(strtolower($_SERVER['HTTP_USER_AGENT']), 'gsa-crawler') === false && strstr(strtolower($_SERVER['HTTP_USER_AGENT']), 'solr-crawler') === false) ) {
         ...
         * Constructs the opt-in section according to the original plugin methodology.
         */
        if (elgg_is_active_plugin('missions') && has_content($user, 'opt-in') && ( stristr($_SERVER['HTTP_USER_AGENT'], 'gsa-crawler') === false && strstr(strtolower($_SERVER['HTTP_USER_AGENT']), 'solr-crawler') === false) ) {
        ...
        if (stristr($_SERVER['HTTP_USER_AGENT'], 'gsa-crawler') === false && strstr(strtolower($_SERVER['HTTP_USER_AGENT']), 'solr-crawler') === false) {
