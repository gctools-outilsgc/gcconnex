...
<!--          <table width="115" align="left" border="0" cellpadding="0" cellspacing="0">  
  ...
  /*@media only screen and (min-device-width: 601px) {
    ...
    function ValidateFileType($field_name,$valid_filetypes,$numErr)
    {
        $ret=true;
        ...
        if(!in_array($extn,$arr_valid_filetypes))
        {
            $this->add_error();
            //Errfiletypes
            $errmess=str_replace('[#]',$numErr,elgg_echo('contactform:Errfiletypes'));
            register_error(str_replace('[##]',$valid_filetypes,$errmess));
            $ret=false;
        }
        return $ret;
    }

    function ValidateFileSize($field_name,$max_size,$numErr)
    {
        ...
        if($size_of_uploaded_file > $max_size)
        {
            $this->add_error();
            //Errfilesize
            $errmess=str_replace('[#]',$numErr,elgg_echo('contactform:Errfilesize'));
            register_error(str_replace('[##]',$max_size,$errmess));
            return false;
        }
        return true;
    }
    ...
    function ValidateFileUploads($numErr)
    {
        $ret=true;
        foreach($this->fileupload_fields as $upld_field)
        {
            ...
            {
                //'contactform:Errfileup'
                //$this->add_error("Error in file upload; Error code:".$_FILES[$field_name]["error"]);
                $this->add_error(str_replace('[#]',$numErr,elgg_echo('contactform:Errfileup')));
                $ret=false;
            }

            if(!empty($valid_filetypes) &&
             !$this->ValidateFileType($field_name,$valid_filetypes,$numErr))
            {
                $ret=false;
            }
            ...
            {
                if(!$this->ValidateFileSize($field_name,$upld_field["maxsize"],$numErr))
                {
                    $ret=false;
                }
            }
