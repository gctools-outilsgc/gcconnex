...
$description_info_en .= 
				"<v:roundrect xmlns:v='urn:schemas-microsoft-com:vml' xmlns:w='urn:schemas-microsoft-com:office:word' href='{$info_val}' style='height:40px;v-text-anchor:middle;width:125px;' arcsize='10%' strokecolor='#1e3650' fillcolor='#46246A'>
					<w:anchorlock/>
...
switch ($msg_type) {
	...
	case 'cp_new_type': // new blogs or other entities
		...
		else if (array_key_exists($vars['cp_topic']->getSubtype(),$entity_etherpad))
			$cp_notify_msg_title_fr = elgg_echo('cp_notify:body_new_content:title_m2',array($topic_author->getURL(), $topic_author->username, $entity_etherpad[$vars['cp_topic']->getSubtype()], $vars['cp_topic']->getURL().'?utm_source=notification&utm_medium=email', $vars['cp_topic']->title2),'fr');
		...
		if ($vars['cp_topic']->getSubtype() === 'groupforumtopic' /*|| $vars['cp_topic']->getContainerEntity() instanceof ElggGroup*/) {
			$cp_notify_msg_description_en = elgg_echo('cp_notify:body_new_content:description_discussion',array("{$cp_topic_description_discussion_en}<br/>", $vars['cp_topic']->getURL().'?utm_source=notification&utm_medium=email'),'en');
			$cp_notify_msg_description_fr = elgg_echo('cp_notify:body_new_content:description_discussion',array("{$cp_topic_description_discussion_fr}<br/>", $vars['cp_topic']->getURL().'?utm_source=notification&utm_medium=email'),'fr');
			...
			$cp_notify_msg_description_en = elgg_echo('cp_notify:body_new_content:description',array("{$cp_topic_description_en}<br/>", $vars['cp_topic']->getURL().'?utm_source=notification&utm_medium=email'),'en');
			$cp_notify_msg_description_fr = elgg_echo('cp_notify:body_new_content:description',array("{$cp_topic_description_fr}<br/>", $vars['cp_topic']->getURL().'?utm_source=notification&utm_medium=email'),'fr');
		}
	...
	case 'cp_site_msg_type': // new messages
		...
		$cp_notify_msg_description_en = elgg_echo('cp_notify:body_site_msg:description_email',array($cp_msg_content,$cp_msg_url.'?utm_source=notification&utm_medium=email'),'en');
		$cp_notify_msg_description_fr = elgg_echo('cp_notify:body_site_msg:description_email',array($cp_msg_content,$cp_msg_url.'?utm_source=notification&utm_medium=email'),'fr');
	...
	case 'cp_group_invite_email':	// inviting non user to group
		...
		$article_link_en = (strpos($site->getURL(), 'gcconnex') !== false) ? "https://gcconnex.gctools-outilsgc.ca/en/support/solutions/articles/2100030466-how-do-i-join-a-group-" : "https://gccollab.gctools-outilsgc.ca/en/support/solutions/articles/2100030475-how-do-i-join-a-group-";
		$article_link_fr = (strpos($site->getURL(), 'gcconnex') !== false) ? "https://gcconnex.gctools-outilsgc.ca/fr/support/solutions/articles/2100030466-how-do-i-join-a-group-" : "https://gccollab.gctools-outilsgc.ca/fr/support/solutions/articles/2100030475-comment-se-joindre-%C3%A0-un-groupe-";
		
		$help_link = (strpos($site->getURL(), 'gcconnex') !== false) ? "https://gcconnex.gc.ca/contactform/?utm_source=notification&utm_medium=email" : "https://gccollab.ca/help/knowledgebase/?utm_source=notification&utm_medium=email";
		$login_link = "{$site->getURL()}login";

		$cp_notify_msg_description_en = elgg_echo('cp_notification:group_invite_email', array($vars['cp_invitation_msg'], $site->name, $invitation_link, $site->name, $registration_link, $site->name, $vars['cp_invitation_code'], $site->name, $login_link, $article_link_en, $help_link), 'en');
		$cp_notify_msg_description_fr = elgg_echo('cp_notification:group_invite_email', array($vars['cp_invitation_msg'], $site->name, $invitation_link, $site->name, $registration_link, $site->name, $vars['cp_invitation_code'], $site->name, $login_link, $article_link_fr, $help_link), 'fr');
