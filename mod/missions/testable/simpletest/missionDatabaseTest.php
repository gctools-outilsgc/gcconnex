...
class MissionDatabaseTest extends ElggCoreUnitTest
{
	...
	private function testMissionEmailMustBeValid($mission)
	{
		$result = $this->assertTrue(
			filter_var($mission->email, FILTER_VALIDATE_EMAIL),
	...
	private function testMissionPhoneMustBeValid($mission)
	{
		if ($mission->phone != '') {
			$result = $this->assertTrue(
				mm_is_valid_phone_number($mission->phone),
	...
	private function testMissionJobTypeMustBeInList($mission)
	{
		...
		$this->assertTrue(
			(array_search($mission->job_type, $list_array) !== false),
	...
	private function testMissionStartMustBeValid($mission)
	{
		...
		if ($result) {
			$this->assertTrue(
				mm_is_valid_date($mission->start_date),
	...
	private function testMissionCompletionMustBeValid($mission)
	{
		if (trim($mission->completion_date) != '') {
			$this->assertTrue(
				mm_is_valid_date($mission->completion_date),
	...
	{
		$this->assertTrue(
			(strtotime($mission->start_date) <= strtotime($mission->completion_date)),
				elgg_echo('missions:diagnostic_suite:start_after_completion', array($mission->guid, $mission->start_date, $mission->completion_date))
		);
		$this->assertTrue(
			(strtotime($mission->deadline) <= strtotime($mission->completion_date)),
	...
	private function testMissionLocationMustBeInList($mission)
	{
		...
		$this->assertTrue(
			(array_search($mission->location, $list_array) !== false),
	...
	private function testMissionSecurityMustBeInList($mission)
	{
		...
		$this->assertTrue(
			(array_search($mission->security, $list_array) !== false || $mission->security === ''),
	...
	private function testMissionTimeIntervalMustBeInList($mission)
	{
		...
		$this->assertTrue(
			(array_search($mission->time_interval, $list_array) !== false),
	...
	private function testMissionTimezoneMustBeInList($mission)
	{
		...
		$this->assertTrue(
			(array_search($mission->timezone, $list_array) !== false || $mission->timezone === ''),
