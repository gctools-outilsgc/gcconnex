	...
	switch ($subtype) {
		...
		case 'discussion_reply':
			...
			if ($entity->getContainerEntity() instanceof ElggGroup) 
				$digest_collection['group']["<a href='{$entity->getContainerEntity()->getURL()}'>{$entity_title}</a>"]['response'][$entity->guid] = json_encode($content_array);
			else 
		...
		default:
			...
			if ($entity->getContainerEntity() instanceof ElggGroup){
				...
				$digest_collection['group']["<a href='{$entity->getContainerEntity()->getURL()}?utm_source=notification_digest&utm_medium=email'>{$entity_title}</a>"]['new_post'][$entity->guid] = json_encode($content_array);
			}else 
				$digest_collection['personal']['new_post'][$entity->guid] = json_encode($content_array);
