...
function create_checkboxes($user_id, $name, $values, $label, $id='chkboxID', $class='chkboxClass')
{
	...
	if (strcmp($name, 'cpn_set_digest_lang_en') == 0) {
		...
		if (!$user_option_en && !$user_option_fr) {
 ...
 * @param string 	$event
 */
function cp_get_headers($event = '')
{	// $event will be null if nothing is passed into it (no default value set)

	$email_address = elgg_get_plugin_setting('cp_notifications_email_addr', 'cp_notifications');
	if (!$email_address || $email_address === '') {
		$email_address = 'admin.gcconnex@tbs-sct.gc.ca';
	}
...
function create_digest($invoked_by, $subtype, $entity, $send_to, $entity_url = '')
{
	...
	if (!$entity->title && ($subtype !== 'single_zip_file_upload' && $subtype !== 'multi_file_upload')) {
	...
	if ($subtype === "hjforumcategory" || $subtype === "hjforum") {
		return true;
	}
...
function has_group_subscriptions($group_guid, $user_guid)
{
	...
	$query = "SELECT R.GUID_ONE,
				...
				LEFT JOIN {$dbprefix}ENTITIES E ON R.GUID_TWO = E.GUID
				LEFT JOIN
					(SELECT GUID
					FROM {$dbprefix}GROUPS_ENTITY
					WHERE GUID = {$group_guid}) G ON E.CONTAINER_GUID = G.GUID
				WHERE R.RELATIONSHIP LIKE 'cp_subscribed_to_%'
					AND E.TYPE = 'object'
					AND E.CONTAINER_GUID = {$group_guid}
					AND R.GUID_ONE = {$user_guid} LIMIT 1
			";
	...
	if (sizeof($subscriptions) == 0) {
		...
		$query = "SELECT ELGG_SUBTYPE.ENTITY_GUID,
					ELGG_SUBTYPE.ENTITY_SUBTYPE
					FROM {$dbprefix}ENTITY_RELATIONSHIPS R
					LEFT JOIN
						(SELECT E.GUID AS ENTITY_GUID,
							S.SUBTYPE AS ENTITY_SUBTYPE
						 FROM {$dbprefix}ENTITIES E, {$dbprefix}ENTITY_SUBTYPES S
						 WHERE (S.SUBTYPE = 'hjforumtopic'
								OR S.SUBTYPE = 'hjforum')
							 AND E.SUBTYPE = S.ID) ELGG_SUBTYPE ON ELGG_SUBTYPE.ENTITY_GUID = R.GUID_TWO
