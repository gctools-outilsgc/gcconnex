...
switch ($msg_type) {
	...
	case 'cp_new_type': // new blogs or other entities
		...
		} elseif (array_key_exists($vars['cp_topic']->getSubtype(), $entity_m)) {
		...
		} elseif (array_key_exists($vars['cp_topic']->getSubtype(), $entity_answer)) {
	...
	// @TODO: can we like user updates? ie avatar change, friend approvals?
		...
		<div>
			<div>
				...
				<div width='100%' style='padding:30px 30px 30px 30px; color:#153643; font-family:sans-serif; font-size:16px; line-height:22px; '>
					...
					</h4> {$cp_notify_msg_description_en}
				</div>
				...
				<div width='100%' style='padding:30px 30px 30px 30px; color:#153643; font-family:sans-serif; font-size:16px; line-height:22px;'>
					...
					</h4> {$cp_notify_msg_description_fr}
				</div>
