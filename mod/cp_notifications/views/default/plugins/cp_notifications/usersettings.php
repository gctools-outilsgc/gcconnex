...
if (strcmp($enable_digest, 'yes') == 0) {
	...
	$chk_email = elgg_view('input/checkbox', array(
						...
						'checked'=> (strcmp($user_option,'set_digest_yes') == 0 || !$user_option) ? true : false,
...
foreach ($personal_notifications as $label) {
	...
	$email_value = (strcmp($plugin->getUserSetting("cpn_{$label}_email", $user->getGUID()),"{$label}_email_none") == 0 || !$email_value) ? false : true;
	$site_value = (strcmp($plugin->getUserSetting("cpn_{$label}_site", $user->getGUID()),"{$label}_site_none") == 0 || !$site_value) ? false : true;
	...
	$chk_sitemail = elgg_view('input/checkbox', array(
	...
	$content .= "<div class='col-sm-2'>{$chk_email}</div> <div class='col-sm-2'>{$chk_sitemail}</div>";
} 
$content .= '</div>';
$content .= '</section>';
