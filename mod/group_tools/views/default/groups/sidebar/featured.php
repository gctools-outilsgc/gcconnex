...
if (!empty($sort) && ($sort == 'alphabetical')) {
	$featured_groups_options['joins'] = ['JOIN ' . elgg_get_config('dbprefix') . 'groups_entity ge ON e.guid = ge.guid'];
...
foreach ($featured_groups as $group) {
	$body .= elgg_view_entity($group, ['full_view' => false]);
}
