		...
		<div class="panel panel-custom">
			<div class="panel-heading">
				...
				<div class="modal-title" id="title">
					<h2><?php echo elgg_echo('geds:personsel:title'); ?></h2>
				</div>
			</div>
			<div class="panel-body">
				<div class="basic-profile-standard-field-wrapper">
					<div id="test"></div>
					<!-- empty div for use later -->
...
require({
		packages: [{
			name: "spin",
			location: '<?php echo elgg_get_site_url(); ?>' + 'mod/geds_sync/vendors',
			main: 'spin'
		}]
	}, ['spin'],
	function(Spinner) {
...
function getFrGEDS() {
	...
	var searchObj = "{\"requestID\" : \"S02\", \"authorizationID\" : \"" + authIDsetting + "\", \"requestSettings\" : {\"searchValue\" : \"" + searchVal + "\", \"searchField\":\"7\", \"searchCriterion\":\"0\"} }";
		...
		success: function(feed) { // feed - JS object - GEDS result
			...
			if (count >= 1) {
				for (personCount in feed.requestResults.personList) { //cycle through people found
					...
					$("#test_inner").append(
						"<button type='button' onclick='setProfile(personObj,jobTitle[personCount],dep[personCount])' class='btn btn-success' data-dismiss='modal' data-toggle='modal' data-target='#gedsSync'> <?php echo elgg_echo('geds:personsel:yes'); ?> </button>"
					);
	...
	<div class="modal-dialog dialog-box">
		<div class="panel panel-custom">
			<div class="panel-heading">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				<div class="modal-title" id="title">
					<h2><?php echo elgg_echo('geds:sync:title'); ?></h2>
				</div>
			</div>
	...
	<div class="modal-dialog dialog-box">
		<div class="panel panel-custom">
			<div class="panel-heading">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				<div class="modal-title" id="title">
					<h2><?php echo elgg_echo('geds:unsync:title'); ?></h2>
				</div>
			</div>
			<div class="panel-body">
				<div class="basic-profile-standard-field-wrapper">
