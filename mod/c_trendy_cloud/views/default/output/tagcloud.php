...
if (!empty($vars['tagcloud']) && is_array($vars['tagcloud'])) {
	...
	foreach ($vars['tagcloud'] as $tag) {
		...
		$tmp_arr = explode(',',$tag->tag);
		$tmp_str = '';
		foreach ($tmp_arr as $ele) {
			$tmp_str .= "<a style='font-size:$size%;text-decoration:none;color:black;' title='$tag->tag ($tag->total)' rel='tag'>$ele</a>";
		}
		$cloud .= $tmp_str;

		// cyu - 03/11/2015: strip the url from the tags
