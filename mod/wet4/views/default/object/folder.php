...
if ($full_view) {
	...
	$body = elgg_view("output/longtext", array("class"=>"folder_desc", "value" => gc_explode_translation($folder->description,$lang)));


	//Identify available content
$description_json = json_decode($folder->description);
$title_json = json_decode($folder->title);
...
if( $description_json->en && $description_json->fr ){
	...
	if (get_current_language() == 'fr'){
		...
		?>			
		<span id="indicator_language_en" onclick="change_en('.elgg-output', '.folder-<?php echo $folder->guid;?>');"><span id="fr_title" class="testClass hidden" ><?php echo $title_json->fr;?></span><span id="en_title" class="testClass hidden" ><?php echo $title_json->en;?></span><span id="en_content" class="testClass hidden" ><?php echo $description_json->en;?></span><span id="fr_content" class="testClass hidden" ><?php echo $description_json->fr;?></span><?php echo elgg_echo('box:indicator:en') ?><span class="fake-link" id="fake-link-1"><?php echo elgg_echo('indicator:click:en') ?></span></span>
	
		<?php
	}else{
		?>		
		<span id="indicator_language_fr" onclick="change_fr('.elgg-output','.folder-<?php echo $folder->guid;?>');"><span id="fr_title" class="testClass hidden" ><?php echo $title_json->fr;?></span><span id="en_title" class="testClass hidden" ><?php echo $title_json->en;?></span><span id="en_content" class="testClass hidden" ><?php echo $description_json->en;?></span><span id="fr_content" class="testClass hidden" ><?php echo $description_json->fr;?></span><?php echo elgg_echo('box:indicator:fr') ?><span class="fake-link" id="fake-link-1"><?php echo elgg_echo('indicator:click:fr') ?></span></span>
