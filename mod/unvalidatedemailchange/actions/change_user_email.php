    ...
    if (!empty($user_guid) && !empty($new_email)) {
        ...
        if (($user instanceof ElggUser) && ($user->canEdit())) {
            ...
            if (validate_email_address($new_email)) {
                ...
                $user->save();
                system_message(elgg_echo(sprintf(elgg_echo('unvalidatedemailchange:user_email_changed_to'), $user->name, $new_email)));
            } else {
                register_error(sprintf(elgg_echo('unvalidatedemailchange:email_address_invalid'), $new_email));
            }
        } else {
