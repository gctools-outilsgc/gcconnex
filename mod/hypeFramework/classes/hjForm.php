...
class hjForm extends ElggObject {
	...
	public function getField($name) {
		$field = elgg_get_entities_from_metadata(array(
			'type' => 'object',
			'subtype' => 'hjfield',
			'container_guid' => $this->guid,
			'limit' => 1,
	...
	public function addField($properties, $guid = null) {
		if ($properties) {
			if ($field = $this->getField($properties['name'])) {
				return $this->updateField($properties, null, $field->guid);
			}
			$field = new hjField($guid);
	...
	public function updateField($properties, $name = null, $guid = null) {
		if (!$name && !$guid) {
			return false;
		}
