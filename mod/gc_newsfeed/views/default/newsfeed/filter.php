...
if(elgg_is_logged_in()){
  ...
  // Temp idea to bring attention to list view!
  $fun_little_badge = '<span class="notif-badge new-badge"><span class="wb-invisible">'.elgg_echo('new').'</span></span>';

  $filter_form = '<a href="#" style="position:absolute; top:10px; right:20px;" title="'.elgg_echo('newsfeed:filter:title').'" class="dropdown  pull-right mrgn-rght-sm" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v fa-2x icon-unsel"><span class="wb-inv">'.elgg_echo('newsfeed:filter:title').'</span></i>'.$fun_little_badge.'</a><ul style="top:45px; right: 8px;" class="dropdown-menu pull-right act-filter" aria-labelledby="dropdownMenu2"><li id="filter_form">'.$filter_link.'</li><li>'.$view_link.'</li></ul>';
