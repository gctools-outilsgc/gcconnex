	...
	if (($group instanceof ElggGroup) && ($group->canEdit() || group_tools_allow_members_invite($group))) {
		...
		if (!empty($user_guids)) {
			if (!$adding) {
					...
					if (!empty($user)) {
						if (!$group->isMember($user)) {
							if (!check_entity_relationship($group->getGUID(), "invited", $user->getGUID()) || $resend) {
								if (c_group_tools_invite_user($group, $user, $text, $resend)) {
									$invited++;
								}
							} else {
				...
				foreach ($user_guids as $u_id) {
					...
					if (!empty($user)) {
						if (!$group->isMember($user)) {
							if (c_group_tools_add_user($group, $user, $text)) {
								$join++;
							}
						} else {
							$member++;
		...
		if (!empty($emails)) {
			foreach ($emails as $email) {
				$invite_result = c_group_tools_invite_email($group, $email, $text, $resend);
