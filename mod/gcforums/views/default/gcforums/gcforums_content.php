...
if ($entity instanceof ElggObject && $entity_guid && $entity->getSubtype() === 'hjforumtopic') {
	...
	if ($nested_forum && !$entity->enable_posting) {	// this was unexpectedly inverted
		echo "<div class='gcforums-display-topics'>".gcforums_topics_list($nested_forum, $group->guid, 1)."</div>";
	}	// display sticky topics

	if ($nested_forum && !$entity->enable_posting) {	// this was unexpectedly inverted
		echo "<div class='gcforums-display-topics'>".gcforums_topics_list($nested_forum, $group->guid, 0)."</div>";
	}	// display unstickied topics
